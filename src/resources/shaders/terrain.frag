...
// Terrain shader - this is where the visual magic happens for the ground
// Spent way too much time tweaking these colors to look natural for Dominica
// The original had this generic terrain look but I wanted something that
// actually feels like tropical Caribbean terrain
// The key was getting the right balance of green tones without making it look fake
...
void main() {
    ...
    float usageFactor = 0.;
    bool hasUsageData = false; // Track if this area has specific land use data
    ...
    if (vMaskUV.x >= 0. && vMaskUV.x <= 1. && vMaskUV.y >= 0. && vMaskUV.y <= 1.) {
        ...
        if (layerIndex != 255.) {
            hasUsageData = true; // This area has specific land use data
    ...
    // GeoTwin: Natural green terrain with subtle variation for unmapped areas
    vec3 baseTerrainColor = detailColor;

    // Add subtle natural texture variation using detail UV and noise
    vec2 naturalUV = vDetailUV * 0.0001; // Very large scale for subtle variation
    float naturalNoise = texture(tDetailNoise, naturalUV).r;

    // Create natural green color variation (tropical vegetation tones)
    vec3 greenVariation1 = vec3(0.79, 0.87, 0.75); // Solid green, closer to deeper tone
    vec3 greenVariation2 = vec3(0.77, 0.85, 0.73); // Very close variation for consistent color

    // Mix green variations based on noise (very subtle)
    vec3 naturalColor = mix(greenVariation2, greenVariation1, smoothstep(0.45, 0.55, naturalNoise));

    // Apply natural green texture to the base terrain
    baseTerrainColor *= naturalColor;

    outColor = vec4(baseTerrainColor, 1);
    outGlow = vec3(0);
    ...
    if (waterFactor > 0.5) {
        ...
        // GeoTwin: Vivid ocean blue water color
        outColor = vec4(0.1, 0.4, 0.8, 0.7);
        outGlow = vec3(0);
