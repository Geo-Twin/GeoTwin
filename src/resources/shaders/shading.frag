...
// GeoTwin: Shadow and SSAO textures removed for performance
...
// GeoTwin: SSR texture removed for performance
...
// GeoTwin: All shadow-related functions removed for performance optimization
...
void main() {
	...
	if (baseColor.a == 0.) {
		...
		// GeoTwin: Brighter daytime sky for better visibility
		vec3 color = (skyColor.rgb + sunLum) * 1.5;
	...
	// GeoTwin: Shadows disabled for performance - always use full lighting
	float shadowFactor = 1.;

	// GeoTwin: Enhanced lighting for better visibility
	Light light = Light(
		sunDirection,
		vec3(1) * sunColor,
		1.4  // Increased intensity for brighter scene
	);

	vec3 color = vec3(0);
	// GeoTwin: SSR disabled for performance - no reflections
	...
	// GeoTwin: Enhanced ambient lighting for better visibility
	color += materialInfo.diffuseColor * 0.35;
	...
	// GeoTwin: Simple brightness boost for better visibility in flood assessment
	color *= 1.8;  // Increase overall scene brightness by 80%
