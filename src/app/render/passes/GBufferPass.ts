...
// Cleaned up - removed old flood material imports
...
export default class GBufferPass extends Pass<{
	...
	// Cleaned up - removed old flood material properties
	...
	private createMaterials(): void {
		...
		// GeoTwin: Cleaned up - using hugging mesh system for flood planes
	}
	...
	private renderExtrudedMeshes(): void {
		...
		for (const tile of tiles) {
			...
			const tileIdUniform = this.extrudedMeshMaterial.getUniform('tileId', 'PerMesh').value;
			if (tileIdUniform instanceof Float32Array || tileIdUniform instanceof Int32Array || tileIdUniform instanceof Uint32Array) {
				tileIdUniform[0] = tile.localId;
			}
	...
	private renderCustomCube(): void {
		const camera = this.manager.sceneSystem.objects.camera;
		const customCube = (this.manager.sceneSystem as any).objects.customCube;
		if (!customCube) return;
		// Use the extruded mesh material for simplicity
		this.renderer.useMaterial(this.extrudedMeshMaterial);
		// Set up the minimal required uniforms
		this.extrudedMeshMaterial.getUniform('projectionMatrix', 'PerMaterial').value = new Float32Array(camera.jitteredProjectionMatrix.values);
		this.extrudedMeshMaterial.updateUniformBlock('PerMaterial');
		const mvMatrix = Mat4.multiply(camera.matrixWorldInverse, customCube.matrixWorld);
		const mvMatrixPrev = Mat4.multiply(this.cameraMatrixWorldInversePrev, customCube.matrixWorld);
		this.extrudedMeshMaterial.getUniform('modelViewMatrix', 'PerMesh').value = new Float32Array(mvMatrix.values);
		this.extrudedMeshMaterial.getUniform('modelViewMatrixPrev', 'PerMesh').value = new Float32Array(mvMatrixPrev.values);
		const tileIdUniform = this.extrudedMeshMaterial.getUniform('tileId', 'PerMesh').value;
		if (tileIdUniform instanceof Float32Array || tileIdUniform instanceof Int32Array || tileIdUniform instanceof Uint32Array) {
	...
	private renderHuggingFloodPlanes(): void {
		const terrain = this.manager.sceneSystem.objects.terrain;
		if (!terrain) return;

		// Use correct huggingMeshMaterial for HuggingFloodPlane objects
		const terrainNormal = <AbstractTexture2DArray>this.getPhysicalResource('TerrainNormal').colorAttachments[0].texture;
		const terrainRingHeight = <AbstractTexture2DArray>this.getPhysicalResource('TerrainRingHeight').colorAttachments[0].texture;
		...
		const camera = this.manager.sceneSystem.objects.camera;
		this.huggingMeshMaterial.getUniform('projectionMatrix', 'PerMaterial').value = new Float32Array(camera.jitteredProjectionMatrix.values);
		this.huggingMeshMaterial.getUniform<UniformFloat1>('time', 'PerMaterial').value[0] = performance.now() * 0.001;
		this.huggingMeshMaterial.updateUniformBlock('PerMaterial');

		// Render dynamic FastFlood simulation flood planes with 3D depth
		this.renderDynamicFastFloodPlanes(terrain);
	}

	private renderHuggingFloodPlane(planeName: string, terrain: any): void {
		const camera = this.manager.sceneSystem.objects.camera;
		const floodPlane = (this.manager.sceneSystem as any).objects[planeName];
		if (!floodPlane) return;
		if (!floodPlane.isMeshReady()) return;

		// Get terrain parameters EXACTLY like regular hugging meshes do
		const tileParams = terrain.getTileParams({
			position: floodPlane.position,
			matrixWorld: floodPlane.matrixWorld
		});
		...
		const {ring0, levelId, ring0Offset, ring1Offset} = tileParams;
		const normalTextureTransforms = this.getTileNormalTexturesTransforms(floodPlane);

		// Use EXACT same setup as regular hugging meshes (lines 404-420 in renderHuggingMeshes)
		const mvMatrix = Mat4.multiply(camera.matrixWorldInverse, floodPlane.matrixWorld);
		const mvMatrixPrev = Mat4.multiply(this.cameraMatrixWorldInversePrev, floodPlane.matrixWorld);
		const relativeCameraPosition = this.getCameraPositionRelativeToTile(camera, floodPlane);

		// Set ALL the same uniforms as regular hugging meshes
		this.huggingMeshMaterial.getUniform('modelViewMatrix', 'PerMesh').value = new Float32Array(mvMatrix.values);
		this.huggingMeshMaterial.getUniform('modelViewMatrixPrev', 'PerMesh').value = new Float32Array(mvMatrixPrev.values);
		this.huggingMeshMaterial.getUniform('transformNormal0', 'PerMesh').value = normalTextureTransforms[0];
		this.huggingMeshMaterial.getUniform('transformNormal1', 'PerMesh').value = new Float32Array([1, 0, 0, 1]);
		this.huggingMeshMaterial.getUniform<UniformFloat1>('terrainRingSize', 'PerMesh').value[0] = ring0.size;
		this.huggingMeshMaterial.getUniform('terrainRingOffset', 'PerMesh').value = new Float32Array([
			ring0Offset.x, ring0Offset.y, ring1Offset.x, ring1Offset.y
		]);
	...
	/**
	 * Render flood plane using EXACT same method as roads (projected mesh approach)
	 */
	private renderRoadLikeFloodPlane(planeName: string, terrain: any): void {
		const camera = this.manager.sceneSystem.objects.camera;
		const floodPlane = (this.manager.sceneSystem as any).objects[planeName];
		if (!floodPlane) return;
		if (!floodPlane.isMeshReady()) return;

		// Get terrain parameters EXACTLY like roads do (from renderProjectedMeshes)
		const tileParams = terrain.getTileParams({
			position: floodPlane.position,
			matrixWorld: floodPlane.matrixWorld
		});
		...
		// Use EXACT same setup as roads (lines 350-370 in renderProjectedMeshes)
		const mvMatrix = Mat4.multiply(camera.matrixWorldInverse, floodPlane.matrixWorld);
		const mvMatrixPrev = Mat4.multiply(this.cameraMatrixWorldInversePrev, floodPlane.matrixWorld);
		const relativeCameraPosition = this.getCameraPositionRelativeToTile(camera, floodPlane);
		const detailTextureOffset = this.getTileDetailTextureOffset(floodPlane);

		// Set ALL the same uniforms as roads (EXACT copy from renderProjectedMeshes)
		this.projectedMeshMaterial.getUniform('modelViewMatrix', 'PerMesh').value = new Float32Array(mvMatrix.values);
		this.projectedMeshMaterial.getUniform('modelViewMatrixPrev', 'PerMesh').value = new Float32Array(mvMatrixPrev.values);
		this.projectedMeshMaterial.getUniform('transformNormal0', 'PerMesh').value = new Float32Array([1, 0, 0, 1]); // Default transform
		this.projectedMeshMaterial.getUniform('transformNormal1', 'PerMesh').value = new Float32Array([1, 0, 0, 1]); // Default transform
		this.projectedMeshMaterial.getUniform<UniformFloat1>('terrainRingSize', 'PerMesh').value[0] = ring0.size;
		this.projectedMeshMaterial.getUniform('terrainRingOffset', 'PerMesh').value = new Float32Array([
			ring0Offset.x, ring0Offset.y, ring1Offset.x, ring1Offset.y
		]);
	...
	public render(): void {
		...
		this.renderCustomCube(); // <--- FORCE RENDER CUSTOM CUBE

		// GeoTwin: Render color-based flood planes LAST to ensure they appear on top of all other features
		this.renderHuggingFloodPlanes();
		// Terrain-based flood visualization disabled - using simple color-based approach
		// this.renderTerrainBasedFloodVisualization();
