...
class WorkerInstance {
	...
	private static instance: WorkerInstance | null = null; // GeoTwin: Static instance reference
	private requestTerrainHeight: boolean = true;
	private straightSkeletonReady: boolean = false;
	private buildingLOD: string = 'lod3'; // GeoTwin: Current building LOD level

	public constructor(private readonly ctx: Worker) {
		WorkerInstance.instance = this; // GeoTwin: Store static reference
		this.addEventListeners();
	}

	// GeoTwin: Static method to get current LOD from anywhere in worker
	public static getCurrentBuildingLOD(): string {
		return WorkerInstance.instance?.buildingLOD || 'lod3';
	}
	...
	private addEventListeners(): void {
		ctx.addEventListener('message', async event => {
			...
			if (data.type === WorkerMessage.ToWorkerType.Start) {
				...
				this.buildingLOD = data.buildingLOD || 'lod3'; // GeoTwin: Update building LOD
...
// GeoTwin: Make WorkerInstance available globally in worker context
(globalThis as any).WorkerInstance = WorkerInstance;
