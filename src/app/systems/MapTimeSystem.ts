...
export default class MapTimeSystem extends System {
	// GeoTwin: Force static morning lighting (9 AM - 12 PM equivalent)
	private state: MapTimeState = MapTimeState.Static;
	private staticLights: [Vec3, Vec3] = StaticPresets[1]; // Morning preset
	...
	public setState(state: number): void {
		// GeoTwin: Always force morning lighting, ignore state changes
		/*
		if (state === 0) {
		...
		*/
		...
		// Force morning lighting regardless of input
		this.state = MapTimeState.Static;
		this.staticLights = StaticPresets[1]; // Morning preset
	}

	private getTargetSunAndMoonDirection(): [Vec3, Vec3] {
		// GeoTwin: Always return static morning lighting, no dynamic calculations
		/*
		switch (this.state) {
		...
		*/
		...
		// Always return morning lighting
		return this.staticLights;
	}

	private updateTargetSkyDirection(): void {
		// GeoTwin: Use static sky direction for consistent daytime lighting
		/*
		...
		*/
		...
		// Static sky direction for consistent daytime lighting
		const px = new Vec3(1, 0, 0);  // East
		const py = new Vec3(0, 1, 0);  // Up
		const pz = new Vec3(0, 0, -1); // North
	...
	// GeoTwin: Commented out time updates for static lighting
	/*
	...
	*/
	...
	public update(deltaTime: number): void {
		// GeoTwin: Static daytime lighting - no dynamic updates
		/*
		this.updateTime();
		...
		*/
		...
		// Initialize static morning lighting once if not set
		if (!this.skyDirection) {
			this.updateTargetSkyDirection();
			this.skyDirection = this.skyDirectionTarget;
			this.updateSkyDirectionMatrix();
		...
		// Static daytime lighting values (9 AM - 12 PM equivalent)
		this.lightIntensity = 10;        // Bright daylight
		this.ambientIntensity = 0.2;     // Good ambient lighting
		this.lightDirection = this.sunDirection;
	}
}