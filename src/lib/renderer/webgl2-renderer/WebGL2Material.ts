	...
	private createUBOs(): void {
		...
		for (const uniform of this.uniforms) {
			if (uniform.block !== null) {
				...
				if (!ubo) {
					console.error(`WebGL2Material: Uniform block '${uniform.block}' not found for uniform '${uniform.name}' in material '${this.name}'`);
					console.error('Available uniform blocks:', Array.from(this.ubos.keys()));
					throw new Error(`Uniform block '${uniform.block}' not found for uniform '${uniform.name}' in material '${this.name}'`);
				}
